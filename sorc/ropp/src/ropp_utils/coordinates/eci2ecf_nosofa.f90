! $Id: eci2ecf_nosofa.f90 2019 2009-01-14 10:20:26Z frhl $

!****f* Coordinates/eci2ecf_nosofa
!
! NAME
!    eci2ecf - Transform coordinates from an intermediate ECI frame
!              to the ECF frame.
!
! SYNOPSIS
!    r_ecf = eci2ecf(year, month, day, hour, minute, sec, dsec, r_eci, SOFA, CIO)
! 
! DESCRIPTION
!    This subroutine calculates coordinates relative to the ECF frame, 
!    given coordinates relative to an intermediate ECI frame.
!    The input ECI frame can be either the classical ECI 'true-of-date' frame
...
! 1.3 Rotation angle, from intermediate ECI frame to ECF
! ------------------------------------------------------
  ...
  if ( xSOFA ) then
    call message (msg_warn, "eci2ecf called with SOFA = .TRUE., " // &
...
! ------------------------
  ...
  do i=1,size(dsec)
    r_ecf(i,:) =  eci2ecf(year, month, day, hour, minute, sec, dsec(i), r_eci(i,:), SOFA, CIO)
  enddo
