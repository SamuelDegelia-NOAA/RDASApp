...
!****s* Interpolation/ropp_fm_spline_2dba_tl *
!
! NAME
!    ropp_fm_spline_2dba_tl - Interpolate logarithmically along a spline for interpolation on finer model grid.
!
! SYNOPSIS
!    call ropp_fm_spline_2dba_tl(x, kspline, array, newx, x_tl, array_tl, interp_tl)
! 
! DESCRIPTION
...
!-------------------------------------------------------------------------------
  ...
  knewlev = SIZE(interp_tl, DIM =1) !klev * kspline
...
! calculate the geometric height of the model levels and interpolate
  ...
  !2c initialisation
    CALL ropp_fm_spline_init_tl(klev,x(:,j),x_tl(:,j),larray(:,j),larray_tl(:,j),y2(:,j),y2_tl(:,j))
  ...
  CALL ropp_fm_splint_tl(klev,knewlev,khoriz,x,x_tl,larray,larray_tl,y2,y2_tl,newx,newx_tl,interp_tl)
